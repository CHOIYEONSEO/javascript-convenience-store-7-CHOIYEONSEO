...
- 입력
  ...
  - [ ] 구매할 상품과 수량이 '상품명-수량'으로 입력되지 않으면 형식이 올바르지 않은 경우로 에러 처리하고 다시 값을 입력 받는다 
  - [ ] 구매할 상품으로 개별 상품이 대괄호로 묶이지 않으면 형식이 올바르지 않은 경우로 에러 처리하고 다시 값을 입력 받는다
  - [ ] 구매할 상품으로 각각의 상품의 쉼표로 구분되지 않으면 형식이 올바르지 않은 경우로 에러 처리하고 다시 값을 입력 받는다
  - [ ] 존재하지 않는 상품을 입력한 경우 에러 처리하고 다시 값을 입력 받는다
  - [ ] 구매 수량이 재고 수량을 초과한 경우 에러 처리하고 다시 값을 입력 받는다
  - [ ] 기타 잘못된 입력의 경우 에러 처리하고 다시 값을 입력 받는다

- 출력
  - [x] 환영 인사와 함께 상품명, 가격, 프로모션 이름, 재고를 안내한다. 만약 재고가 0개라면 재고 없음을 출력한다.
  - [ ] 구매할 상품과 수량 형식이 올바르지 않은 경우: '[ERROR] 올바르지 않은 형식으로 입력했습니다. 다시 입력해 주세요.'를 출력한다
...
- 입력
  - [ ] 구현에 필요한 행사 목록을 파일 입출력을 통해 불러온다.
    - [ ] 과 public/promotions.md 파일을 이용한다.
