...
describe("Receipt 테스트", () => {
    test("구매 상품 내역을 추가한다", () => {
        ...
        expect(receipt.getProducts()).toEqual([value1, value2, value3]);
    })
    ...
    // receipt의 구매 상품 내역 : [["사이다", 5, 1000], ["콜라", 3, 1700], ["물", 3, 1400]]
    test("총 구매 수량을 반환한다", () => {
        expect(receipt.getTotalNumber()).toBe(11);
    })

    test("총 구매액을 반환한다", () => {
        expect(receipt.getTotalPrice()).toBe(14300);
    })
